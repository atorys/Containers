...
void test_vector()
{
	...
	std::cout << "TEST 1 assert\n";
	{
		...
		std::vector<char>::allocator_type *p_alloc = (std::allocator<char> *) 0;
		std::vector<char>::pointer p_ptr = (char *) 0;
		std::vector<char>::const_pointer p_cptr = (const char *) 0;
		std::vector<char>::reference p_ref = ch;
		std::vector<char>::const_reference p_cref = (const char &) ch;
		std::vector<char>::value_type *p_val = (char *) 0;
		std::vector<char>::size_type *p_size = (size_t *) 0;
		std::vector<char>::difference_type *p_diff = (ptrdiff_t *) 0;
		(void) p_cref;
		(void) p_val;
		(void) p_ptr;
		(void) p_diff;
		...
		std::vector<char> v1a(6, 'x');


		std::vector<char> v1b(7, 'y', al);
		...
		std::vector<char> v2(v1a);
		assert (v2.size() == 6 && v2.front() == 'x');


		std::vector<char> v3(v1a.begin(), v1a.end());
		assert(v3.size() == 6 && v3.front() == 'x');


		std::vector<char> v4(v1a.begin(), v1a.end());
		...
		std::vector<char>::iterator p_it(vector.begin());
		std::vector<char>::const_iterator p_cit(v4.begin());
		std::vector<char>::reverse_iterator p_rit(vector.rbegin());
		std::vector<char>::const_reverse_iterator p_crit(v4.rbegin());
		...
		std::vector<std::pair<int, char>, std::allocator<std::pair<int, char> > > pair;

		pair.insert(pair.begin(), std::make_pair(4, 'a'));

		std::vector<std::pair<int, char>, std::allocator<std::pair<int, char> > >::iterator itp(pair.begin());
...
void	test_stack() {
	...
	{
		std::stack<int, std::deque<int> > stack_deq_buffer;
	}
...
void test_map() {
	...
	{
		std::map<int, std::string >	v0;
		std::cout << "empty map created size=" << v0.size() << " maxsize=" << v0.max_size() << "\n";
		for (int i = 0; i < 100; i++)
			v0.insert(std::make_pair(i, std::to_string(i)));
		std::cout << "fill map newsize=" << v0.size() << "\n";

		std::map<int, std::string>	v0a(v0);
		assert(v0 == v0a);
	...
	std::cout << "TEST 3 capacity\n\n";
	{
		...
		for (int i = 0; i < 1000; i += 2)
			v0.insert(std::make_pair(i, i + i % 4));
	...
	std::cout << "TEST 4 access and iterators\n\n";
	{
		...
		std::map<int, int >::iterator	it = v0.begin();
		for (; it != v0.end(); ++it)
			std::cout << it->first << " ";
		std::cout << "\nbackward iteration : ";
		--it;
		...
		std::cout << "forward iteration : ";
		std::map<int, int >::reverse_iterator	it2 = v0.rbegin();
		for (; it2 != v0.rend(); ++it2)
			std::cout << it2->first << " ";
		std::cout << "\nbackward iteration : ";
		--it2;
	...
	{
		std::map<char, char >	v0;
		std::map<char, char >	v0a;
		std::map<char, char >	v0b;

		std::cout << "first insertion with insert(VALUE) \n";
		for (int i = 0; i < 100; i += 3) {
			std::cout << (v0.insert(std::make_pair(i, i + i % 2)).second ? "+" : "-");
		}
		...
		for (int i = 38; i < 88; ++i)
			std::cout << v0a.insert(v0a.begin(), std::make_pair(i, i))->first;
		std::cout <<"\n";
...
void test_set()
{
	...
	std::cout << "TEST 2 constructor\n\n";
	{
		std::set<int, std::greater<int> >	v0;
		std::set<int, std::less<int> >		v2;
		...
		std::set<int, std::greater<int> >	v0a(v0);
		assert(v0 == v0a);
		...
		std::set<int, std::greater<int> >	v0b = v0;
	...
	std::cout << "TEST 4 access and iterators\n\n";
	{
		...
		std::cout << "created non empty map, random access iterator \n";
		std::cout << "forward iteration : ";
		std::set<int>::iterator	it = v0.begin();
		for (; it != v0.end(); ++it)
			std::cout << *it << " ";
		std::cout << "\nbackward iteration : ";
		--it;
		...
		std::cout << "iteration with reverse iterator \n";
		std::cout << "forward iteration : ";
		std::set<int>::reverse_iterator	it2 = ++v0.rbegin();
		for (; it2 != v0.rend(); ++it2)
			std::cout << *it2 << " ";
		std::cout << "\nbackward iteration : ";
		--it2;
